...
int main()
{
    ...
    using namespace ExampleSocketUtils;

    std::string time_str;
    Logger logger_("examples_connections.log");
    auto tcpServerRecvCallback = [&logger_](TCPSocket *socket, Nanos rx_time) noexcept
    {
    ...
    auto tcpClientRecvCallback = [&logger_](TCPSocket *socket, Nanos rx_time) noexcept
    {
        ...
        logger_.log("TCPSocket::defaultRecvCallback() socket:% len:% rx:% msg:% \n", socket->fd_,
                    socket->next_rcv_valid_index_, rx_time, recv_msg);
    };
