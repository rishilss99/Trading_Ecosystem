...
namespace CustomExchange
{
    ...
    {
    public:
        OrderServer(ClientRequestLFQueue *client_requests, ClientResponseLFQueue *client_responses, std::string &iface, int port);
        ~OrderServer();
        ...
        // Main run loop for order server thread
        auto run() noexcept
        {
            ...
            if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest))
            {
                ...
                for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_; i += sizeof(OMClientRequest))
                {
                    ...
                    logger_.log("%:% %() % Received %\n", __FILE__, __LINE__,
                                ...
                                request->toString());
                    if (UNLIKELY(cid_tcp_socket_[request->me_client_request_.client_id_] == nullptr)) // First message from this client
                    {
                    ...
                    if (cid_tcp_socket_[request->me_client_request_.client_id_] != socket)
                    {
                        logger_.log("%:% %() % Received ClientRequest from ClientId : % on different socket : % expected : %\n ", __FILE__, __LINE__,
                                    __FUNCTION__, Common::getCurrentTimeStr(&time_str_), request->me_client_request_.client_id_,
                    ...
                    if (next_exp_seq_num != request->seq_num_)
                    {
