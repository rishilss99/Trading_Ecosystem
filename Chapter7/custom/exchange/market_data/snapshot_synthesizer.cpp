...
namespace CustomExchange
{
    SnapshotSynthesizer::SnapshotSynthesizer(MDPMarketUpdateLFQueue *market_updates, const std::string &iface,
                                             const std::string &snapshot_ip, int snapshot_port) : snapshot_md_updates_(market_updates), logger_("exchange_snapshot_synthesizer.log"),
    ...
    auto SnapshotSynthesizer::publishSnapshot()
    {
        ...
        const MDPMarketUpdate start_market_update{snapshot_size++, {.type_ = MarketUpdateType::SNAPSHOT_START, .order_id_ = last_inc_seq_num_}};
        ...
        for (size_t ticker_id = 0; ticker_id < ticker_orders_.size(); ticker_id++)
        {
            ...
            const MDPMarketUpdate clear_market_update{snapshot_size++, {.type_ = MarketUpdateType::CLEAR, .ticker_id_ = ticker_id}};
        ...
        const MDPMarketUpdate end_market_update{snapshot_size++, {.type_ = MarketUpdateType::SNAPSHOT_END, .order_id_ = last_inc_seq_num_}};
