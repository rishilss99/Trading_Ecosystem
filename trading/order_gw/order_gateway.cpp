...
namespace CustomTrading {
  ...
  auto OrderGateway::run() noexcept -> void {
    ...
    while (run_) {
      ...
      for(auto client_request = outgoing_requests_->getNextToRead(); client_request; client_request = outgoing_requests_->getNextToRead()) {
        ...
        tcp_socket_.send(client_request, sizeof(CustomExchange::MEClientRequest));
  ...
  auto OrderGateway::recvCallback(TCPSocket *socket, Nanos rx_time) noexcept -> void {
    ...
    if (socket->next_rcv_valid_index_ >= sizeof(CustomExchange::OMClientResponse)) {
      size_t i = 0;
      for (; i + sizeof(CustomExchange::OMClientResponse) <= socket->next_rcv_valid_index_; i += sizeof(CustomExchange::OMClientResponse)) {
        auto response = reinterpret_cast<const CustomExchange::OMClientResponse *>(socket->inbound_data_.data() + i);
