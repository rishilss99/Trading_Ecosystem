...
namespace CustomTrading {
  MarketDataConsumer::MarketDataConsumer(Common::ClientId client_id, CustomExchange::MEMarketUpdateLFQueue *market_updates,
  ...
  auto MarketDataConsumer::checkSnapshotSync() -> void {
    ...
    if (first_snapshot_msg.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_START) {
    ...
    std::vector<CustomExchange::MEMarketUpdate> final_events;
    ...
    for (auto &snapshot_itr: snapshot_queued_msgs_) {
      ...
      if (snapshot_itr.second.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_START &&
          snapshot_itr.second.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_END)
    ...
    if (last_snapshot_msg.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_END) {
    ...
    for (auto inc_itr = incremental_queued_msgs_.begin(); inc_itr != incremental_queued_msgs_.end(); ++inc_itr) {
      ...
      if (inc_itr->second.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_START &&
          inc_itr->second.type_ != CustomExchange::MarketUpdateType::SNAPSHOT_END)
  ...
  auto MarketDataConsumer::queueMessage(bool is_snapshot, const CustomExchange::MDPMarketUpdate *request) {
    if (is_snapshot) {
  ...
  auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void {
    ...
    if (socket->next_rcv_valid_index_ >= sizeof(CustomExchange::MDPMarketUpdate)) {
      size_t i = 0;
      for (; i + sizeof(CustomExchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(CustomExchange::MDPMarketUpdate)) {
        auto request = reinterpret_cast<const CustomExchange::MDPMarketUpdate *>(socket->inbound_data_.data() + i);
        logger_.log("%:% %() % Received % socket len:% %\n", __FILE__, __LINE__, __FUNCTION__,
                    Common::getCurrentTimeStr(&time_str_),
                    (is_snapshot ? "snapshot" : "incremental"), sizeof(CustomExchange::MDPMarketUpdate), request->toString());
