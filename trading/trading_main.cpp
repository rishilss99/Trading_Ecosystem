...
CustomTrading::TradeEngine *trade_engine = nullptr;
CustomTrading::MarketDataConsumer *market_data_consumer = nullptr;
CustomTrading::OrderGateway *order_gateway = nullptr;
...
int main(int argc, char **argv) {
  ...
  trade_engine = new CustomTrading::TradeEngine(client_id, algo_type,
                                          ticker_cfg,
                                          &client_requests,
                                          &client_responses,
                                          &market_updates);
  ...
  order_gateway = new CustomTrading::OrderGateway(client_id, &client_requests, &client_responses, order_gw_ip, order_gw_iface, order_gw_port);
  order_gateway->start();
  ...
  market_data_consumer = new CustomTrading::MarketDataConsumer(client_id, &market_updates, mkt_data_iface, snapshot_ip, snapshot_port, incremental_ip, incremental_port);
  ...
  if (algo_type == AlgoType::RANDOM) {
    ...
    std::vector<CustomExchange::MEClientRequest> client_requests_vec;
    ...
    for (size_t i = 0; i < 10000; ++i) {
      ...
      CustomExchange::MEClientRequest new_request{CustomExchange::ClientRequestType::NEW, client_id, ticker_id, order_id++, side,
                                            price, qty};
      ...
      cxl_request.type_ = CustomExchange::ClientRequestType::CANCEL;
