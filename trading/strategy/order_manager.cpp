...
namespace CustomTrading {
  ...
  auto OrderManager::newOrder(OMOrder *order, TickerId ticker_id, Price price, Side side, Qty qty) noexcept -> void {
    const CustomExchange::MEClientRequest new_request{CustomExchange::ClientRequestType::NEW, trade_engine_->clientId(), ticker_id,
  ...
  auto OrderManager::cancelOrder(OMOrder *order) noexcept -> void {
    const CustomExchange::MEClientRequest cancel_request{CustomExchange::ClientRequestType::CANCEL, trade_engine_->clientId(),
