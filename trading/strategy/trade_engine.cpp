...
namespace CustomTrading {
  ...
  auto TradeEngine::sendClientRequest(const CustomExchange::MEClientRequest *client_request) noexcept -> void {
  ...
  auto TradeEngine::onTradeUpdate(const CustomExchange::MEMarketUpdate *market_update, MarketOrderBook *book) noexcept -> void {
    logger_.log("%:% %() % %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
                market_update->toString().c_str());
  ...
  auto TradeEngine::onOrderUpdate(const CustomExchange::MEClientResponse *client_response) noexcept -> void {
    logger_.log("%:% %() % %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
                client_response->toString().c_str());

    if (UNLIKELY(client_response->type_ == CustomExchange::ClientResponseType::FILLED)) {
