...
namespace CustomTrading {
  ...
  public:
    TradeEngine(Common::ClientId client_id,
                ...
                CustomExchange::ClientRequestLFQueue *client_requests,
                CustomExchange::ClientResponseLFQueue *client_responses,
                CustomExchange::MEMarketUpdateLFQueue *market_updates);
    ...
    auto sendClientRequest(const CustomExchange::MEClientRequest *client_request) noexcept -> void;
    ...
    auto onTradeUpdate(const CustomExchange::MEMarketUpdate *market_update, MarketOrderBook *book) noexcept -> void;

    /// Process client responses - updates the position keeper and informs the trading algorithm about the response.
    auto onOrderUpdate(const CustomExchange::MEClientResponse *client_response) noexcept -> void;
    ...
    std::function<void(const CustomExchange::MEMarketUpdate *market_update, MarketOrderBook *book)> algoOnTradeUpdate_;
    std::function<void(const CustomExchange::MEClientResponse *client_response)> algoOnOrderUpdate_;
  ...
  private:
    ...
    CustomExchange::ClientRequestLFQueue *outgoing_ogw_requests_ = nullptr;
    CustomExchange::ClientResponseLFQueue *incoming_ogw_responses_ = nullptr;
    CustomExchange::MEMarketUpdateLFQueue *incoming_md_updates_ = nullptr;
    ...
    auto defaultAlgoOnTradeUpdate(const CustomExchange::MEMarketUpdate *market_update, MarketOrderBook *) noexcept -> void {
      logger_.log("%:% %() % %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
                  market_update->toString().c_str());
    }

    auto defaultAlgoOnOrderUpdate(const CustomExchange::MEClientResponse *client_response) noexcept -> void {
      logger_.log("%:% %() % %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
