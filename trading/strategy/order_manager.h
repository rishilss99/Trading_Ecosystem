  ...
  public:
    ...
    auto onOrderUpdate(const CustomExchange::MEClientResponse *client_response) noexcept -> void {
      ...
      switch (client_response->type_) {
        case CustomExchange::ClientResponseType::ACCEPTED: {
          order->order_state_ = OMOrderState::LIVE;
        }
          break;
        case CustomExchange::ClientResponseType::CANCELED: {
          order->order_state_ = OMOrderState::DEAD;
        }
          break;
        case CustomExchange::ClientResponseType::FILLED: {
          order->qty_ = client_response->leaves_qty_;
          if(!order->qty_)
            order->order_state_ = OMOrderState::DEAD;
        }
          break;
        case CustomExchange::ClientResponseType::CANCEL_REJECTED:
        case CustomExchange::ClientResponseType::INVALID: {
        }
          break;
      }
    }
