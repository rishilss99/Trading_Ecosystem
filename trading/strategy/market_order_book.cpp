...
namespace CustomTrading {
  ...
  auto MarketOrderBook::onMarketUpdate(const CustomExchange::MEMarketUpdate *market_update) noexcept -> void {
    ...
    switch (market_update->type_) {
      case CustomExchange::MarketUpdateType::ADD: {
        ...
        break;
      case CustomExchange::MarketUpdateType::MODIFY: {
        auto order = oid_to_order_.at(market_update->order_id_);
        order->qty_ = market_update->qty_;
      }
        break;
      case CustomExchange::MarketUpdateType::CANCEL: {
        auto order = oid_to_order_.at(market_update->order_id_);
        removeOrder(order);
      }
        break;
      case CustomExchange::MarketUpdateType::TRADE: {
        trade_engine_->onTradeUpdate(market_update, this);
        return;
      }
        break;
      case CustomExchange::MarketUpdateType::CLEAR: { // Clear the full limit order book and deallocate MarketOrdersAtPrice and MarketOrder objects.
        ...
        break;
      case CustomExchange::MarketUpdateType::INVALID:
      case CustomExchange::MarketUpdateType::SNAPSHOT_START:
      case CustomExchange::MarketUpdateType::SNAPSHOT_END:
        break;
    }
